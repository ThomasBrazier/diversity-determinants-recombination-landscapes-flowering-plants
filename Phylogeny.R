...
#============================================================================#
#       Building the phylogenetic tree data - Old way with iTOL ----
#============================================================================#
# Classify species
# Grey branches for species with only one of either genetic map or genome
# Black branches for species with both maps but not processed
# Red branches for validated Marey maps, i.e. the recombination map exists
# List of species with a genome at chromosome level
list.genomes = as.character(unique(genome$species))
# Filtering for crossings
list.genomes = list.genomes[!grepl("_x_", list.genomes)]
# List of species with a genetic map ((i.e. maps in /data/Genetic_maps)
list.geneticmap = as.character(metadata$species[!is.na(metadata$id)])
# List of all validated genetic maps
list.mareymap = as.character(metadata.clean$species)
# A vector of all species integrated in the phylogenetic tree
list.tree = unique(c(list.genomes, list.geneticmap, list.mareymap))
# Remove genera without species name
list.tree = list.tree[!grepl("_sp", list.tree)]
# Filtering species
list.tree = list.tree[!(list.tree %in% c("Ostreococcus_lucimarinus"))]

# Affecting colors in a vector
colours = rep("Grey", length(list.tree))
# If the species is both in list.genomes and list.geneticmap, then it will be printed in black
sum((list.tree %in% list.genomes) & (list.tree %in% list.geneticmap))
colours[((list.tree %in% list.genomes) & (list.tree %in% list.geneticmap))] = "Black"
# If the species is in list.mareymap, then it will be printed in red
colours[list.tree %in% list.mareymap] = "Red"

# Retrieve the iTOL phylogenetic tree for species in our dataset
list.tree = gsub("_", " ", list.tree)
list.tree = list.tree[!list.tree == "Fragaria ananassa"] # Exception: not working for this species
# Matching names in iTOL
(resolved_names = tnrs_match_names(list.tree))
save(resolved_names, file = "output/phylogeny/resolved_names.Rdata")
load("output/phylogeny/resolved_names.Rdata")
# Trimming
# resolved_names = resolved_names[!(resolved_names$ott_id == 1064791),]
tree = tol_induced_subtree(ott_ids = ott_id(resolved_names))
...
tips = gsub("_ott[0-9]*", "", tips)
tips = gsub("_\\(species_in_domain_Eukaryota\\)", "", tips)
# Affecting colors to tips in a vector
colours = rep("Grey", length(tips))
# If the species is both in list.genomes and list.geneticmap, then it will be printed in black
sum((tips %in% list.genomes) & (tips %in% list.geneticmap))
colours[((tips %in% list.genomes) & (tips %in% list.geneticmap))] = "Black"
# If the species is in list.mareymap, then it will be printed in red
colours[tips %in% list.mareymap] = "Red"

#============================================================================#
#       Making a circular phylogenetic tree with project advances ----
#============================================================================#
# Assign groups to the tip labels
tree = groupOTU(tree, list(grey = which(colours == "Grey"),
                            black = which(colours == "Black"),
                            red = which(colours == "Red")))
# Correct tiplabels
tips2 = gsub("_", " ", tips)
tree$tip.label = tips2

p = ggtree(tree, aes(color = group),layout = 'fan', open.angle = 10) +
  geom_tiplab(size = 3) +
  scale_color_manual(values = c("Black", "Grey", "Red")) +
  theme(legend.position = "none")
p

ggsave("figures/Maps_phylogeny.png",
       device="png",dpi=320,units="cm",width=40,height=40)


#============================================================================#
#       Mapping the estimated mean adn SD of recombination rates on a phylogenetic tree ----
#============================================================================#

# Affecting recombination rates to tips in a vector
mean_rec_rate = rep(NA, length(tips))
sd_rec_rate = rep(NA, length(tips))
# The mean recombination of the species is affected to the gradient vector
for (i in 1:length(tips)) {
  mean_rec_rate[i] = mean(chromosome.stats$mean.recrate[regmatches(as.character(chromosome.stats$set), gregexpr("^[A-Z][a-z]+_[a-z]+", as.character(chromosome.stats$set))) == tips[i]], na.rm = TRUE)
  sd_rec_rate[i] = mean(chromosome.stats$sd.recrate[regmatches(as.character(chromosome.stats$set), gregexpr("^[A-Z][a-z]+_[a-z]+", as.character(chromosome.stats$set))) == tips[i]], na.rm = TRUE)
}

# tmp = tibble(label = as.phylo(tree)$tip.label, trait = mean_rec_rate)
# full_join(tree, tmp, by="label")

p2 = ggtree(tree, layout = 'fan', open.angle = 10) +
  geom_tiplab(size = 3, offset = 10) +
  theme(legend.position = "none")
# p2

df = data.frame(mean = mean_rec_rate)
rownames(df) = tree$tip.label
p3 = gheatmap(p2, df, offset=.8, width=.1,
              colnames_angle=90, colnames_offset_y = 218.5) +
              scale_fill_viridis_c(option="D", name = "Mean")
# p3

df2 = data.frame(SD = sd_rec_rate)
rownames(df2) = tree$tip.label
p4 = p3 + new_scale_fill()
p4 = gheatmap(p4, df2, offset=4, width=.1,
         colnames_angle=90, colnames_offset_y = 218.5) +
         scale_fill_viridis_c(option="A", name = "SD")
p4

ggsave("figures/Maps_phylogeny_MeanRecRate.png",
       device="png",dpi=320,units="cm",width=40,height=40)
...
#============================================================================#
#       Building a reduced phylogenetic tree, subset only species with a map ----
#============================================================================#
...
list.tree = gsub("_", " ", list.tree)
list.tree = list.tree[!list.tree == "Fragaria ananassa"] # Exception: not working for this species
list.tree = gsub("Quercus sp", "Quercus robur", list.tree)
...
save(tree, file = "output/phylogeny/tree_reduced.Rdata")
...
p = ggtree(tree, layout = 'fan', branch.length = "none", open.angle = 10) +
  geom_tiplab(size = 3) +
  scale_color_manual(values = c("Black", "Grey", "Red")) +
  xlim(-40, 40) +
  theme(legend.position = "none")
...
ggsave("figures/Maps_phylogeny_reduced.png",
       device="png",units="cm", width=15, height=15)
